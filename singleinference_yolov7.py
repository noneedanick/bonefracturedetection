...
class SingleInference_YOLOV7:
    ...
    def load_cv2mat(self,im0=None):
        ...
        if type(self.im0)!=type(None):
            ...
            self.imn = cv2.cvtColor(self.imn, cv2.COLOR_RGB2Lab)
            self.imn[:,:,0] = clahe.apply(self.imn[:,:,0])
            self.imn = cv2.cvtColor(self.imn, cv2.COLOR_Lab2RGB)
...
if __name__=='__main__':  
    ...
    path_img_i=r"test_images/fracture_elbow.jpg"
    #path_img_i=r"/media/steven/Elements/Drone_Videos_Park/FPS_DESIRED_1d5/JPEGImages/DJI_0028_fps24_frame00000040.jpg"

    #INITIALIZE THE app
    app=SingleInference_YOLOV7(img_size,path_yolov7_weights,path_img_i,device_i='cpu',conf_thres=0.15,iou_thres=0.5)
